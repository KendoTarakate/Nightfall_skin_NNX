...
/**
 * Chunked skin upload/download networking.
 *
 * <p>Loader-agnostic: built on Architectury's {@link NetworkManager}, so the exact
 * same protocol implementation is shared by Fabric and Forge. The packet payloads
 * are identical to the original Fabric implementation.
 */
public class SkinNetworkHandler {
    ...
    // Max image dimension kept when uploading to a server. Skins larger than this are downscaled;
    // 1024x1024 is the supported maximum, so valid skins pass through untouched.
    private static int maxMultiplayerSize = 1024;
    // Hard safety cap on the uploaded PNG byte size. A 1024x1024 PNG is well under this, so valid
    // skins are NEVER re-compressed/downscaled (fully lossless); this only guards against absurd files.
    private static final int MAX_UPLOAD_BYTES = 8 * 1024 * 1024; // 8 MB
    ...
    public static void registerClientReceivers() {
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.S2C, SKIN_LIST_RESPONSE, (buf, context) -> {
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.S2C, DOWNLOAD_SKIN_START, (buf, context) -> {
            ...
            context.queue(() -> receivingDownloads.put(playerUuid, new ChunkedSkinData(totalChunks, isSlim)));
        });

        // Receive skin chunk from server
        NetworkManager.registerReceiver(NetworkManager.Side.S2C, DOWNLOAD_SKIN_CHUNK, (buf, context) -> {
            UUID playerUuid = buf.readUuid();
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.S2C, DOWNLOAD_SKIN_END, (buf, context) -> {
            UUID playerUuid = buf.readUuid();
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.S2C, PLAYER_SKIN_RESET, (buf, context) -> {
            UUID playerUuid = buf.readUuid();
            NightfallSkin.LOGGER.info("Player {} reset their skin", playerUuid);

            context.queue(() -> SkinManager.removeRemoteSkin(playerUuid));
        });
    }

    public static void registerServerReceivers() {
        // CLIENT -> SERVER: Upload skin start
        NetworkManager.registerReceiver(NetworkManager.Side.C2S, UPLOAD_SKIN_START, (buf, context) -> {
            ...
            context.queue(() -> receivingUploads.put(senderUuid, new ChunkedSkinData(totalChunks, isSlim)));
        });

        // CLIENT -> SERVER: Upload skin chunk
        NetworkManager.registerReceiver(NetworkManager.Side.C2S, UPLOAD_SKIN_CHUNK, (buf, context) -> {
            ServerPlayerEntity player = (ServerPlayerEntity) context.getPlayer();
            UUID senderUuid = player.getUuid();
            int chunkIndex = buf.readInt();
            int chunkSize = buf.readInt();
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.C2S, UPLOAD_SKIN_END, (buf, context) -> {
            ServerPlayerEntity player = (ServerPlayerEntity) context.getPlayer();
            UUID senderUuid = player.getUuid();
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.C2S, REQUEST_SKIN_LIST, (buf, context) -> {
            ServerPlayerEntity player = (ServerPlayerEntity) context.getPlayer();
            ...
            context.queue(() -> {
                ...
                NetworkManager.sendToPlayer(player, SKIN_LIST_RESPONSE, response);
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.C2S, REQUEST_SKIN, (buf, context) -> {
            ServerPlayerEntity player = (ServerPlayerEntity) context.getPlayer();
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.C2S, RESET_SKIN, (buf, context) -> {
            ServerPlayerEntity player = (ServerPlayerEntity) context.getPlayer();
            UUID senderUuid = player.getUuid();
                        ...
                        for (ServerPlayerEntity targetPlayer : server.getPlayerManager().getPlayerList()) {
                            if (!targetPlayer.getUuid().equals(senderUuid)) {
                                try {
                                    ...
                                    NetworkManager.sendToPlayer(targetPlayer, PLAYER_SKIN_RESET, resetBuf);
                                } catch (Exception e) {
    ...
    public static void uploadSkinToServer(byte[] imageData, boolean isSlim) {
        try {
            ...
            // Further compression ONLY as a last-resort safety net for absurdly large files.
            while (pngData.length > MAX_UPLOAD_BYTES && image.getWidth() > 64) {
    ...
    private static void sendChunkedUpload(byte[] data, boolean isSlim) {
        ...
        NetworkManager.sendToServer(UPLOAD_SKIN_START, startBuf);
        ...
        for (int i = 0; i < totalChunks; i++) {
            ...
            NetworkManager.sendToServer(UPLOAD_SKIN_CHUNK, chunkBuf);
        }

        // Send end
        PacketByteBuf endBuf = new PacketByteBuf(Unpooled.buffer());
        NetworkManager.sendToServer(UPLOAD_SKIN_END, endBuf);
    ...
    private static void sendSkinToPlayer(ServerPlayerEntity player, UUID skinOwnerUuid, byte[] data, boolean isSlim) {
        try {
            ...
            startBuf.writeInt(totalChunks);
            startBuf.writeInt(data.length);
            NetworkManager.sendToPlayer(player, DOWNLOAD_SKIN_START, startBuf);
            ...
            for (int i = 0; i < totalChunks; i++) {
                ...
                chunkBuf.writeInt(chunkSize);
                chunkBuf.writeBytes(data, offset, chunkSize);
                NetworkManager.sendToPlayer(player, DOWNLOAD_SKIN_CHUNK, chunkBuf);
            }

            // Send end
            PacketByteBuf endBuf = new PacketByteBuf(Unpooled.buffer());
            endBuf.writeUuid(skinOwnerUuid);
            NetworkManager.sendToPlayer(player, DOWNLOAD_SKIN_END, endBuf);
    ...
    public static void requestSkinList() {
        ...
        NetworkManager.sendToServer(REQUEST_SKIN_LIST, buf);
    ...
    public static void requestSkinFromServer(UUID playerUuid) {
        ...
        NetworkManager.sendToServer(REQUEST_SKIN, buf);
    ...
    public static void sendSkinReset() {
        try {
            PacketByteBuf buf = new PacketByteBuf(Unpooled.buffer());
            NetworkManager.sendToServer(RESET_SKIN, buf);
